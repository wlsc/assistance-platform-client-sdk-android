...
public class LoginFacebook extends AbstractLogin implements ILoginData {

	private static LoginFacebook m_instance;
	...
	public static void onSessionStateChange(Session session, SessionState state, Exception exception) {
		try {
			...
			if (state.isOpened()) {
				...
				LoginFacebook.getInstance().handleReceivedToken(session.getAccessToken(), null, m_ctxActivity);
